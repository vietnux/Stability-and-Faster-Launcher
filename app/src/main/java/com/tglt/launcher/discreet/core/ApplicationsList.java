...
import com.tglt.launcher.discreet.storage.* ;
...
public class ApplicationsList
{
	...
	private static ArrayList<Application> quickbar ;
	private boolean update_in_progress ;
	static ArrayList<Application> defaultApps = new ArrayList<>() ;
...
//			if( DefaultApps.checkDefaultApps(context, entry, onlyTypes) ) {
//				quickbar_default.add(application);
//			}
		...
		if(quickbar.isEmpty()) {
//			ArrayList<Application> quickbar_default_sort = new ArrayList<>() ;
//			for( String type: onlyTypes) {
//				for( Application a: quickbar_default) {
//					if ( a.name.toLowerCase().contains(type.toLowerCase()) || a.apk.toLowerCase().contains(type.toLowerCase()) )
//						quickbar_default_sort.add(a); quickbar_default.remove(a); break;
//				}
//			}

				mStartupTask = new StartupTask(true, 0);
	...
	public static ArrayList<Application> setQuickBar() {
		ArrayList<Application> quickbar_default = new ArrayList<>() ;
		List<String> onlyTypes = new ArrayList<>();
		onlyTypes.add("phone");
		onlyTypes.add("msg");
		//		onlyTypes.add("music");
		//		onlyTypes.add("browser");
		//		onlyTypes.add("email");
		//		onlyTypes.add("camera");
		for(Application app : defaultApps) {
			if( DefaultApps.checkDefaultApps(ActivityMain.getContext(), app, onlyTypes) ) {
				quickbar_default.add(app);
			}
		}
		return quickbar_default;
	}
	public static void setAppQuickBar(ArrayList<Application> quickbar_default) {
		for( Application asort: quickbar_default)
			if( quickbar.size() < Constants.COLUMNS_QUICK_PORTRAIT ) {
				quickbar.add(asort);
			}
	}
	...
	/**
	 * Update the Quick Bar applications list based on the Quick bar file and the complete list.
	 */
	public void updateQuickBar() {
		// Initializations
		quickbar.clear() ;
		ArrayList<String> quickbar_file = new InternalFileTXT(Constants.FILE_QUICK_BAR).readAllLines() ;
		if(quickbar_file == null) return ;

		// Browse the internal file
		for(String line : quickbar_file) {
			// Search the internal name in the applications list
			for(Application application : getApplications(true))
				if(application.getName().equals(line))
					{
						// Add the application to the quickbar and move to the next line
						if(!quickbar.contains(application)) quickbar.add(application) ;
						break ;
					}
		}
	}
	...
	public ArrayList<Application> getQuickbar()
	{
		return quickbar ;
	}
	...
	private static class StartupTask extends AsyncTask<Void,Integer,ArrayList<Application> > {
		...
		StartupTask(boolean showProgress, int startAtStage) {
		...
		@Override
		protected ArrayList<Application> doInBackground(Void... voids) {
			try {
				return ApplicationsList.setQuickBar();
			} catch (Throwable t) {
				Log.e("All App Quick", t.getMessage(), t);
			} finally {
			}
			return null;
		}
		...
		@Override
		protected void onPostExecute(ArrayList<Application> quickbar_default) {
			ApplicationsList.setAppQuickBar(quickbar_default);
			ActivityMain.getMainActivity().updateAdaptersQuick();
		}
	}
}
